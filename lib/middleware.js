...
var noopMiddleware = function(config, handlerBuilder, callback) {
  handlerBuilder(config, callback)
}

var loadMiddlewareDependencies = function(middlewareKey, dependencies, config, handlerBuilder, callback) {
  ...
  var innerHandlerBuilder = function(config, callback) {
    loadMiddlewareDependencies(middlewareKey, restDependencies, config, handlerBuilder, callback)
  }
...
var createMiddlewareFromFilter = function(options) {
  var middlewareName = options.name
  var allowRepeat = options.allowRepeat
  var dependencies = options.dependencies || [ ]

  var filter = options.filter || noopFilter
  var handlerType = options.handlerType || 'stream'
  var middlewareKey = handlerType + '-middlewares'
  ...
  var middleware = function(config, handlerBuilder, callback) {
    ...
    loadMiddlewareDependencies(middlewareKey, dependencies, config, filteredHandlerBuilder, callback)
  }

  return middleware
}

var createInputHandleableMiddleware = function(options) {
  var middlewareName = options.name
  var handleableName = options.name
  var allowRepeat = options.allowRepeat
  var inputHandlerBuilder = options.inputHandlerBuilder
  ...
  var middleware = function(config, handlerBuilder, callback) {
    ...
    var filteredHandlerBuilder = function(config, callback) {
      ...
      config.inputHandleables = config.inputHandleables || { }
      if(config.inputHandleables[handleableName]) return handlerBuilder(config, callback)

      inputHandlerBuilder(copyObject(config), function(err, handler) {
        if(err) return callback(err)
        ...
        config.inputHandleables[handleableName] = handleable
        handlerBuilder(config, callback)
      })
    }
...
var createStreamMiddleware = function(options) {
  options.handlerType = 'stream'
  return createMiddlewareFromFilter(options)
}

var createHttpMiddleware = function(options) {
  options.handlerType = 'http'
  return createMiddlewareFromFilter(options)
}
...
module.exports = {
  ...
  createInputHandleableMiddleware: createInputHandleableMiddleware,
